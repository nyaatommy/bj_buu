...
sub my_status_mobile {
	...
	if ($m{wea}) {
		...
		elsif ($weas[$m{wea}][2] =~ /風|炎|雷|銃/)    { $sub_mat = "+$weas[$m{wea}][3]"; $sub_ag = "-$weas[$m{wea}][5]"; }
	...
	if ($m{gua}) {
		...
		elsif ($guas[$m{gua}][2] =~ /風|炎|雷|銃/)    { $sub_mdf = "+$guas[$m{gua}][3]"; $sub_ag .= "-$guas[$m{gua}][5]"; }
	}
